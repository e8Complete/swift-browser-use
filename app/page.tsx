...
import { EnterIcon, LoadingIcon, MicrophoneIcon, StopCircleIcon } from "@/lib/icons";
...
// --- Agent State Types ---
type AgentStatus = "idle" | "starting" | "transcribing" | "running" | "paused" | "done_success" | "done_failure" | "error" | "disconnected" | "unknown";
...
export default function Home() {
    ...
    const [formSubmissionState, formAction] = useFormState(handleSubmitAction, { status: "idle" });
    const isSubmitting = formSubmissionState.status === 'submitting';

	const vad = useMicVAD({
		startOnLoad: false,
		onSpeechEnd: (audio) => {
			if (agentStatus !== "idle" && agentStatus !== 'error' && agentStatus !== 'done_success' && agentStatus !== 'done_failure') {
	...
	const connectWebSocket = useCallback(() => {
		...
		if (sessionId && !wsRef.current) {
            ...
            if (!wsUrlFromEnv) {
                console.error("Error: NEXT_PUBLIC_PYTHON_WS_URL environment variable is not set.");
            ...
            try {
                ...
                ws.onclose = (event) => {
                    ...
                    if (agentStatus !== 'done_success' && agentStatus !== 'done_failure' && agentStatus !== 'error') {
    ...
    useEffect(() => {
        ...
        return () => {
            if (wsRef.current) {
                console.log("Closing WebSocket due to effect cleanup (unmount/sessionId change).");
    ...
    async function handleSubmitAction(
        prevState: { status: string; error?: string; transcript?: string; sessionId?: string },
        formData: FormData
    ): Promise<{ status: string; error?: string; transcript?: string; sessionId?: string }> {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                toast.error(errorText || `Failed to start agent task (${response.status})`);
                setAgentStatus("error");
            ...
            return { status: "error", error: error.message || "Unknown submission error" };
        }
    }

	function handleTextFormSubmit(e: React.FormEvent) {
		e.preventDefault();
         if (!input.trim() || isSubmitting || (agentStatus !== 'idle' && agentStatus !== 'error' && agentStatus !== 'done_success' && agentStatus !== 'done_failure')) {
             console.log("Submission prevented: Input empty, submitting, or agent busy.");
    ...
    const isAgentBusy = isSubmitting || agentStatus === 'running' || agentStatus === 'starting' || agentStatus === 'transcribing';
		...
		<>
				...
				<button onClick={() => setSessionId('test-123')}>Connect WS Test</button>
					<h2 className="text-lg font-semibold flex items-center gap-2">
                        ...
                        <span className={`font-mono text-xs px-2 py-0.5 rounded font-medium ${
                            agentStatus === 'running' ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200' :
                            agentStatus === 'done_success' ? 'bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-200' :
                            agentStatus === 'done_failure' || agentStatus === 'paused' ? 'bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-200' :
                            agentStatus === 'error' || agentStatus === 'disconnected' ? 'bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-200' :
                    ...
                    {sessionId && <p className="text-xs text-neutral-500 dark:text-neutral-500">Session: {sessionId.substring(0, 8)}...</p>}
                    <div className="text-sm text-neutral-700 dark:text-neutral-300 space-y-1 pt-2">
                         <p><strong>Step:</strong> {agentStep > 0 ? agentStep : "N/A"}</p>
                         <p><strong>Goal:</strong> {currentGoal || (agentStatus === 'idle' ? "Waiting for task..." : "...")}</p>
                         <p><strong>Action:</strong> {lastAction || "None"}</p>
                         {finalResult && <p className="font-medium mt-3 pt-3 border-t border-neutral-200 dark:border-neutral-800"><strong>Result:</strong> {finalResult}</p>}
                    </div>
				</div>
				<div className="w-full md:w-1/2 lg:w-3/5 border border-neutral-200 dark:border-neutral-800 rounded-md overflow-hidden bg-neutral-100 dark:bg-neutral-900 flex items-center justify-center aspect-[16/10] md:aspect-auto">
					{screenshotData ? (
						...
						<span className="text-neutral-500 text-sm p-4">
                             {agentStatus === 'running' ? 'Waiting for screenshot...' : 'No screenshot'}
                        </span>
					)}
				</div>
			</div>
			...
			>
				<input
					...
					required
					placeholder={
                        vad.listening ? "Listening via microphone..." :
                        isAgentBusy ? "Agent is running..." :
				...
				>
                    {isSubmitting || agentStatus === 'starting' ? <LoadingIcon /> : <EnterIcon />}
				</button>
                 ...
                 >
                    {vad.loading ? <LoadingIcon/> : (vad.listening ? <StopCircleIcon /> : <MicrophoneIcon />)}
                 </button>
			</form>

			<div className="text-neutral-500 dark:text-neutral-500 pt-4 text-center max-w-xl text-balance min-h-28 text-xs space-y-2">
                 {lastTranscript && <p className="italic">You said: "{lastTranscript}"</p>}

				{agentStatus === 'idle' && !lastTranscript && (
					<>
						<p className="font-medium">
							Speak (press mic) or type a task.
						</p>
						{vad.loading ? (
							<p>Loading speech detection...</p>
						) : (
							<p>Mic Status: {vad.listening ? "Listening..." : (vad.userSpeaking ? "Processing..." : "Ready")}</p>
						)}
					</>
				)}
                {formSubmissionState.status === 'error' && <p className="text-red-500">Submission Error: {formSubmissionState.error}</p>}
			</div>
			...
			<div
				className={clsx(
					"fixed bottom-10 left-1/2 -translate-x-1/2 size-24 blur-2xl rounded-full bg-gradient-to-b from-blue-300 to-blue-500 dark:from-blue-700 dark:to-blue-900 -z-10 transition-all duration-300 ease-in-out",
					{
						...
						"opacity-30": !vad.loading && !vad.userSpeaking && !vad.listening && agentStatus === 'idle',
						"opacity-50 scale-100": vad.listening && !vad.userSpeaking && agentStatus === 'idle',
